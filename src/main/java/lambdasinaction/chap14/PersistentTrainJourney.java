...
public class PersistentTrainJourney {
  ...
  static TrainJourney link(TrainJourney a, TrainJourney b) {
    ...
    TrainJourney t = a;
    // из первого поезда А находим его последнюю станцию
    // и ей добавляем ещё один пункт назначения В
    while (t.onward != null) {
      t = t.onward;
    }
    //This works by finding the last leg in the TrainJourney for a
    // and replacing the null marking the end of a’s list with list b
    // (you need a special case if a has no elements).
    t.onward = b;
    return a;
  }
