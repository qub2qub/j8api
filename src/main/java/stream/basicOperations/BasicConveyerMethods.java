...
public class BasicConveyerMethods {
    ...
    public static void main(String[] args) {
        ...
        //filter
        System.out.println(stringList.stream().filter("Mark"::equals).count());
        System.out.println(stringList.stream().filter("Mark"::equals).collect(Collectors.toList()));

        //skip - skipe N first elements
        System.out.println(stringList.stream().skip(stringList.size()-3).findFirst().orElse("-1"));
        ...
        System.out.println(stringList.stream().map(s->s+"_1").collect(Collectors.toList()));
        ...
        int[] ints = stringStream.mapToInt(s -> Integer.parseInt(s)-1).toArray();
        for (int anInt : ints) {
        ...
        System.out.println(stringList.stream()
                .flatMap(s -> Arrays.asList(s.split("o")).stream()).collect(Collectors.toList()));

        long[] longs = stringStream1
                .flatMapToLong(s -> Collections.singletonList(Long.parseLong(s))
